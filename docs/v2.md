...
- Use `collab config --tui` or keep using existing config commands. Default models,
  ...
  Text edits offer a temporary file in your configured terminal editor, or inline
  input. `r` / **Reset to default** in the TUI and `--unset` in the CLI restore
  defaults. `collab config --help` lists these choices.
