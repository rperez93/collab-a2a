...
not have a watcher — review at safe task boundaries, and read the next section.
...
- `no active collab session` means this workspace and agent namespace has no
  active session. Check the workspace, stable agent identity, and any explicit
  `COLLAB_HOME`; changing agent sessions does not adopt another owner’s state.
